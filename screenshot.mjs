/**
 * screenshot.mjs — AICC screenshot utility
 *
 * Usage:
 *   node screenshot.mjs                              → http://localhost:3000, auto-label
 *   node screenshot.mjs http://localhost:3000 faq    → saves as screenshot-N-faq.png
 *   node screenshot.mjs http://localhost:3000 faq --sel=#faq  → scroll to element first
 *
...
const viewportWidth = process.argv[4] && !process.argv[4].startsWith("--") ? parseInt(process.argv[4]) : 1440;
const viewportHeight = process.argv[5] && !process.argv[5].startsWith("--") ? parseInt(process.argv[5]) : 900;
...
const sel = selArg ? selArg.split("=")[1] : "";
